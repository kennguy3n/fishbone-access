...
func netskopeValidConfig() map[string]interface{} { return map[string]interface{}{} }
func netskopeValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "netskope_demo_token"}
}

func TestNetskopeConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Netskope-Api-Token") == "" {
			t.Errorf("Netskope-Api-Token header missing")
		}
	...
	cfg := netskopeValidConfig()
	secrets := netskopeValidSecrets()
...
func TestNetskopeConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		netskopeValidConfig(), netskopeValidSecrets(),
		access.AccessGrant{UserExternalID: "alice@example.com", ResourceExternalID: "tenant_admin"})
