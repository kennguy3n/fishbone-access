...
func TestDeelGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestDeelGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://app.deel.com/sso/saml/acme/metadata",
		"sso_entity_id":    "https://app.deel.com/sso/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
