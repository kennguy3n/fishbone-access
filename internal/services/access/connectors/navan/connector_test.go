...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "nv_AAAA1234bbbbCCCC"}
}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				items = append(items, map[string]interface{}{"id": fmt.Sprintf("u%d", i), "email": fmt.Sprintf("u%d@x.com", i), "firstName": "U", "lastName": fmt.Sprintf("%d", i), "status": "ACTIVE"})
			}
			body["users"] = items
		} else {
			if page != "1" {
			...
			body["users"] = []map[string]interface{}{{"id": "ulast", "email": "last@x.com", "firstName": "L", "lastName": "ast", "status": "INACTIVE"}}
		}
