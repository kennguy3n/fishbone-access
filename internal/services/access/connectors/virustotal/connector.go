...
// /api/v3/groups membership surface (see advanced.go); this sentinel
// is kept for any future capability VirusTotal does not expose (e.g.
// SCIM, group sync) so callers do not have to special-case the
// package name.
var ErrNotImplemented = fmt.Errorf("virustotal: capability not supported by this connector: %w", access.ErrCapabilityNotSupported)
