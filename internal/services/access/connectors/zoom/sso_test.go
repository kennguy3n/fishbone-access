...
func TestZoomGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestZoomGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://acme.zoom.us/saml/metadata",
		"sso_entity_id":    "https://acme.zoom.us/saml",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
