...
func TestSurveysparrowGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestSurveysparrowGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://api.surveysparrow.com/saml/acme/metadata",
		"sso_entity_id":    "https://api.surveysparrow.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
