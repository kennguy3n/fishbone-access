...
func surveysparrowValidConfig() map[string]interface{} { return map[string]interface{}{} }
func surveysparrowValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "surveysparrow_demo"}
}

func TestSurveysparrowConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		teamPath := "/v3/team"
		memberPath := "/v3/team/" + memberID
		emailPath := "/v3/team/" + email
		mu.Lock()
		defer mu.Unlock()
		switch {
		case r.Method == http.MethodPost && r.URL.Path == teamPath:
			if state != "" {
	...
	cfg := surveysparrowValidConfig()
	secrets := surveysparrowValidSecrets()
...
func TestSurveysparrowConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		surveysparrowValidConfig(), surveysparrowValidSecrets(),
