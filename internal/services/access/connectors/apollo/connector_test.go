...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"api_key": "apollo_AAAA1234bbbbCCCC"}
}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				items = append(items, map[string]interface{}{"id": fmt.Sprintf("u%d", i), "name": fmt.Sprintf("U%d", i), "email": fmt.Sprintf("u%d@x.com", i)})
			}
			body["users"] = items
		} else {
			if page != "2" {
			...
			body["users"] = []map[string]interface{}{{"id": "ulast", "name": "Last", "email": "last@x.com"}}
		}
