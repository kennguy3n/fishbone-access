...
func TestMixpanelGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestMixpanelGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://mixpanel.com/sso/saml/acme/metadata",
		"sso_entity_id":    "https://mixpanel.com/sso/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
