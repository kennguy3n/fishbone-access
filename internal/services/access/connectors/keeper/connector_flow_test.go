...
func keeperValidConfig() map[string]interface{} { return map[string]interface{}{} }
func keeperValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "keeper_demo"}
}

func TestKeeperConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		usersPath := "/api/rest/users"
		userPath := "/api/rest/users/" + userID
		emailPath := "/api/rest/users/" + email
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := keeperValidConfig()
	secrets := keeperValidSecrets()
...
func TestKeeperConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		keeperValidConfig(), keeperValidSecrets(),
