...
func copyaiValidConfig() map[string]interface{} { return map[string]interface{}{} }
func copyaiValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "copyai_demo"}
}

func TestCopyAIConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		membersPath := "/api/v1/workspace/members"
		memberPath := "/api/v1/workspace/members/" + memberID
		emailPath := "/api/v1/workspace/members/" + email
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := copyaiValidConfig()
	secrets := copyaiValidSecrets()
...
func TestCopyAIConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		copyaiValidConfig(), copyaiValidSecrets(),
