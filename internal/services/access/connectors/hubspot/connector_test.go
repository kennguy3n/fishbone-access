...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"access_token": "pat-na1-aaaaBBBB1111"}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		page++
		if page == 1 {
			_, _ = w.Write([]byte(`{"results":[{"id":"1","email":"a@b.com","firstName":"Alice"}],"paging":{"next":{"after":"AFT","link":"/x"}}}`))
			return
		}
		if r.URL.Query().Get("after") != "AFT" {
			t.Errorf("after = %q", r.URL.Query().Get("after"))
		}
		_, _ = w.Write([]byte(`{"results":[{"id":"2","email":"b@b.com","firstName":"Bob"}],"paging":{}}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
...
func TestListEntitlements_HappyPath(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"roleIds":["r1","r2"]}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
	c.urlOverride = srv.URL
