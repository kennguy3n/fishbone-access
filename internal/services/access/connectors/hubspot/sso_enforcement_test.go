...
func TestCheckSSOEnforcement_Enforced(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !strings.HasSuffix(r.URL.Path, "/settings/v3/users/provisioning") {
			t.Errorf("path=%q; want suffix /settings/v3/users/provisioning", r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"ssoEnabled":true,"ssoRequired":true}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
...
func TestCheckSSOEnforcement_NotEnforced(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"ssoEnabled":true,"ssoRequired":false}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
	c.urlOverride = srv.URL
