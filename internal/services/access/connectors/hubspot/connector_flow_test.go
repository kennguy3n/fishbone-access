...
// TestConnectorFlow_FullLifecycle walks HubSpot's
// /settings/v3/users/{id}/roles[/{role}] PUT/DELETE round trip.
func TestConnectorFlow_FullLifecycle(t *testing.T) {
	var hasRole atomic.Bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodPut && strings.HasSuffix(r.URL.Path, "/u-1/roles"):
			hasRole.Store(true)
			w.WriteHeader(http.StatusNoContent)
		case r.Method == http.MethodDelete && strings.Contains(r.URL.Path, "/u-1/roles/role-1"):
			hasRole.Store(false)
			w.WriteHeader(http.StatusNoContent)
		case r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/users/u-1"):
			if hasRole.Load() {
				_, _ = w.Write([]byte(`{"roleIds":["role-1"]}`))
				return
			}
