...
func TestOVHcloudGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestOVHcloudGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://eu.api.ovh.com/saml/acme/metadata",
		"sso_entity_id":    "https://eu.api.ovh.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
