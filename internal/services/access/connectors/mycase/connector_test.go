...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "myc_AAAA1234bbbbCCCC"}
}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				arr = append(arr, map[string]interface{}{"id": fmt.Sprintf("u%d", i), "email": fmt.Sprintf("u%d@x.com", i), "first_name": fmt.Sprintf("U%d", i), "last_name": fmt.Sprintf("U%d", i), "active": true})
			}
		} else {
			if page != "2" {
				t.Errorf("page = %q", page)
			}
			arr = []map[string]interface{}{{"id": "ulast", "email": "last@x.com", "first_name": "Last", "last_name": "Last", "active": true}}
		}
		body["data"] = arr
