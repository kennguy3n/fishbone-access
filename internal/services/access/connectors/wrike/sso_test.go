...
func TestWrikeGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestWrikeGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://www.wrike.com/saml/acme/metadata",
		"sso_entity_id":    "https://www.wrike.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
