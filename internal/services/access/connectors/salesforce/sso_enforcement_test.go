...
func TestCheckSSOEnforcement_Enforced(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !strings.Contains(r.URL.Path, "/tooling/query") {
			t.Errorf("path=%q; want tooling/query", r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"records":[{"RequireSingleSignOn":true}]}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
...
func TestCheckSSOEnforcement_NotEnforced(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"records":[{"RequireSingleSignOn":false}]}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
	c.urlOverride = srv.URL
