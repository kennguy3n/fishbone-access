...
func TestRapid7GetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestRapid7GetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://insight.rapid7.com/saml/acme/metadata",
		"sso_entity_id":    "https://insight.rapid7.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
