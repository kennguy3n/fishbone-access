...
const (
	appfolioAuditPageSize = 100
	appfolioAuditMaxPages = 200
)

// FetchAccessAuditLogs streams AppFolio Voyager audit events into the
...
func (c *AppFolioAccessConnector) FetchAccessAuditLogs(
	...
	var collected []appfolioAuditEvent
	for page := 0; page < appfolioAuditMaxPages; page++ {
		if err := ctx.Err(); err != nil {
			return err
		}
		q := url.Values{}
		q.Set("per_page", fmt.Sprintf("%d", appfolioAuditPageSize))
		...
		if err != nil {
			return fmt.Errorf("appfolio: audit: %w", err)
		}
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			return fmt.Errorf("appfolio: audit: status %d: %s", resp.StatusCode, string(body))
		}
		var envelope appfolioAuditPage
		if err := json.Unmarshal(body, &envelope); err != nil {
			return fmt.Errorf("appfolio: decode audit: %w", err)
		}
		collected = append(collected, envelope.Data...)
		if len(envelope.Data) < appfolioAuditPageSize {
			break
		}
	}
	...
	for i := range collected {
		entry := mapAppFolioAuditEvent(&collected[i])
		if entry == nil {
			continue
		}
...
type appfolioAuditEvent struct {
	ID        string `json:"id"`
...
type appfolioAuditPage struct {
	Data []appfolioAuditEvent `json:"data"`
}
...
func parseAppFolioAuditTime(s string) time.Time {
...
var _ access.AccessAuditor = (*AppFolioAccessConnector)(nil)
