...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "sxg_AAAA1234bbbbCCCC"}
}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				arr = append(arr, map[string]interface{}{"id": fmt.Sprintf("u%d", i), "email": fmt.Sprintf("u%d@x.com", i), "name": fmt.Sprintf("U%d", i), "status": "active"})
			}
		} else {
			if page != "2" {
				t.Errorf("page = %q", page)
			}
			arr = []map[string]interface{}{{"id": "ulast", "email": "last@x.com", "name": "Last", "status": "active"}}
		}
		body["data"] = arr
