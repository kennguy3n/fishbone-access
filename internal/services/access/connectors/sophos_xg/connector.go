...
func (c *SophosXGAccessConnector) do(req *http.Request) ([]byte, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("sophos_xg: %s %s: status %d: %s", req.Method, req.URL.Path, resp.StatusCode, httputil.SafeErrorBody(body))
	}
	return body, nil
}
...
type sophosXgListResponse struct {
	Items []sophosXgUser `json:"data"`
}
