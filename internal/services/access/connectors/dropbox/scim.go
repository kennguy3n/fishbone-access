// Package dropbox — SCIM v2.0 outbound provisioning composition.
//
// Dropbox Business publishes a SCIM 2.0 endpoint at
// `{base}/2/team/scim/v2/Users` (and /Groups). The shared SCIMClient
// drives it with the same Bearer team token the connector already
// uses for the team members API.
package dropbox
...
func (c *DropboxAccessConnector) scimConfig(configRaw, secretsRaw map[string]interface{}) (map[string]interface{}, map[string]interface{}, error) {
	_, secrets, err := c.decodeBoth(configRaw, secretsRaw)
	if err != nil {
		return nil, nil, err
	}
	scimBaseURL := c.baseURL() + "/2/team/scim/v2"
...
func (c *DropboxAccessConnector) PushSCIMUser(ctx context.Context, configRaw, secretsRaw map[string]interface{}, user access.SCIMUser) error {
...
func (c *DropboxAccessConnector) PushSCIMGroup(ctx context.Context, configRaw, secretsRaw map[string]interface{}, group access.SCIMGroup) error {
	scimCfg, scimSecrets, err := c.scimConfig(configRaw, secretsRaw)
	if err != nil {
		return err
	}
...
func (c *DropboxAccessConnector) DeleteSCIMResource(ctx context.Context, configRaw, secretsRaw map[string]interface{}, resourceType, externalID string) error {
	scimCfg, scimSecrets, err := c.scimConfig(configRaw, secretsRaw)
	if err != nil {
		return err
	}
...
var _ access.SCIMProvisioner = (*DropboxAccessConnector)(nil)
