...
func jasperValidConfig() map[string]interface{} { return map[string]interface{}{} }
func jasperValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "jasper_demo"}
}

func TestJasperConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		membersPath := "/v1/team/members"
		memberPath := "/v1/team/members/" + memberID
		emailPath := "/v1/team/members/" + email
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := jasperValidConfig()
	secrets := jasperValidSecrets()
...
func TestJasperConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		jasperValidConfig(), jasperValidSecrets(),
