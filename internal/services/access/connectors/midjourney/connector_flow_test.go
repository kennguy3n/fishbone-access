...
func midjourneyValidConfig() map[string]interface{} { return map[string]interface{}{} }
func midjourneyValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "midjourney_demo"}
}

func TestMidjourneyConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		memberPath := "/api/v1/members/" + memberID
		emailPath := "/api/v1/members/" + email
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := midjourneyValidConfig()
	secrets := midjourneyValidSecrets()
...
func TestMidjourneyConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		midjourneyValidConfig(), midjourneyValidSecrets(),
