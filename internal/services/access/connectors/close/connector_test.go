...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"api_key": "api_AAAA1234bbbbCCCC"}
}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				items = append(items, map[string]interface{}{"id": fmt.Sprintf("user_%d", i), "first_name": "U", "last_name": fmt.Sprintf("%d", i), "email": fmt.Sprintf("u%d@x.com", i)})
			}
			body["data"] = items
			body["has_more"] = true
		} else {
			...
			body["data"] = []map[string]interface{}{{"id": "user_last", "first_name": "L", "last_name": "ast", "email": "last@x.com"}}
			body["has_more"] = false
		}
