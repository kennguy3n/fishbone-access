...
func validConfig() map[string]interface{} { return map[string]interface{}{"site": "example"} }
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"api_key": "tok_AAAA1234bbbbCCCC"}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		wrapped := make([]map[string]interface{}, 0, len(arr))
		for _, c := range arr {
			wrapped = append(wrapped, map[string]interface{}{"customer": c})
		}
		body := map[string]interface{}{"list": wrapped}
		if calls == 1 {
			body["next_offset"] = "page2-offset"
		}
