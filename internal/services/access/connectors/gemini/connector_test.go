...
func TestValidate_RejectsMissing(t *testing.T) {
	...
	if err := c.Validate(context.Background(), map[string]interface{}{"project_id": "BAD UPPER"}, validSecrets()); err == nil {
		t.Error("expected error for invalid project_id")
	}
	if err := c.Validate(context.Background(), map[string]interface{}{"project_id": "abc"}, validSecrets()); err == nil {
		t.Error("expected error for too-short project_id")
	}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/v1/projects/shieldnet360-prod/users" {
...
func TestGetCredentialsMetadata_RedactsToken(t *testing.T) {
	...
	if md["project_id"] != "shieldnet360-prod" {
		t.Errorf("project_id = %v", md["project_id"])
	}
}
