...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"access_token": "concur_AAAA1234bbbbCCCC"}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/api/v3.0/common/users" {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				items = append(items, map[string]interface{}{"ID": fmt.Sprintf("u%d", i), "PrimaryEmail": fmt.Sprintf("u%d@x.com", i), "FirstName": "U", "LastName": fmt.Sprintf("%d", i), "Active": true})
			}
			body["Items"] = items
		} else {
			...
			body["Items"] = []map[string]interface{}{{"ID": "ulast", "PrimaryEmail": "last@x.com", "FirstName": "L", "LastName": "ast", "Active": false}}
		}
