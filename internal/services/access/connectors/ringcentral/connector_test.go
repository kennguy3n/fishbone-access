...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Query().Get("perPage"); got == "" {
			t.Errorf("perPage missing; query=%q", r.URL.RawQuery)
		}
		...
		if calls == 1 {
			for i := 0; i < pageSize; i++ {
				arr = append(arr, map[string]interface{}{
					...
					"status":  "Enabled",
					"contact": map[string]interface{}{"email": fmt.Sprintf("u%d@x.com", i)},
				})
			}
		} else {
			arr = []map[string]interface{}{{"id": 9999, "name": "Last", "status": "Disabled", "contact": map[string]interface{}{"email": "last@x.com"}}}
		}
		body := map[string]interface{}{"records": arr}
