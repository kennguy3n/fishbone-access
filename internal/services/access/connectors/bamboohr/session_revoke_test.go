...
func TestRevokeUserSessions_HappyPath(t *testing.T) {
	...
	if err := c.RevokeUserSessions(context.Background(), validConfig(), validSecrets(), "42"); err != nil {
		t.Fatalf("RevokeUserSessions: %v", err)
	}
	if seenMethod != http.MethodPut {
		t.Errorf("method=%q; want PUT", seenMethod)
	}
	if !strings.HasSuffix(seenPath, "/v1/employees/42/terminateEmployee") {
		t.Errorf("path=%q; want suffix /v1/employees/42/terminateEmployee", seenPath)
	}
}
