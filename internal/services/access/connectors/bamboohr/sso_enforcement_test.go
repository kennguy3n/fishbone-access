...
func TestCheckSSOEnforcement_Enforced(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !strings.HasSuffix(r.URL.Path, "/v1/meta/security") {
			t.Errorf("path=%q; want /v1/meta/security", r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"sso_only":true,"sso":{"enabled":true}}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
...
func TestCheckSSOEnforcement_NotEnforced(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"sso_only":false,"sso":{"enabled":true}}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
	c.urlOverride = srv.URL
