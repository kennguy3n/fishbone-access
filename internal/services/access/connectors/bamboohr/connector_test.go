...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"api_key": "bhAAAA1234bbbbCCCC"}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"employees":[{"id":"1","displayName":"Alice","firstName":"Alice","lastName":"A","workEmail":"alice@x.com","status":"Active"},{"id":"2","displayName":"Bob","workEmail":"bob@x.com","status":"Inactive"}]}`))
	}))
	t.Cleanup(srv.Close)
	c := New()
...
func TestGetSSOMetadata(t *testing.T) {
	...
	if md == nil || !strings.Contains(md.MetadataURL, "acme.bamboohr.com") {
		t.Errorf("md = %+v", md)
	}
}
