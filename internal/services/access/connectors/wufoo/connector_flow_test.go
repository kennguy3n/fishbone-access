...
func wufooValidConfig() map[string]interface{} {
	return map[string]interface{}{"subdomain": "acme"}
}
func wufooValidSecrets() map[string]interface{} {
	return map[string]interface{}{"api_key": "wufoo_demo_key"}
}

func TestWufooConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		usersPath := "/api/v3/users.json"
		userPath := "/api/v3/users/" + userID + ".json"
		emailPath := "/api/v3/users/" + email + ".json"
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := wufooValidConfig()
	secrets := wufooValidSecrets()
...
func TestWufooConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		wufooValidConfig(), wufooValidSecrets(),
