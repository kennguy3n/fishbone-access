...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "sk-AAAA1234bbbbCCCC"}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			body["has_more"] = true
			body["last_id"] = fmt.Sprintf("user-%d", pageSize-1)
		} else {
			if after == "" {
				t.Errorf("second call after = empty; want non-empty")
			}
			arr = []map[string]interface{}{{"id": "user-last", "email": "last@x.com", "name": "Last", "role": "owner"}}
			body["has_more"] = false
			body["last_id"] = "user-last"
		}
		body["data"] = arr
