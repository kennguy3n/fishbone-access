...
func bitsightValidConfig() map[string]interface{} { return map[string]interface{}{} }
func bitsightValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "bitsight_demo"}
}

func TestBitSightConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		usersPath := "/ratings/v1/users"
		userPath := "/ratings/v1/users/" + userID
		emailPath := "/ratings/v1/users/" + email
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := bitsightValidConfig()
	secrets := bitsightValidSecrets()
...
func TestBitSightConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		bitsightValidConfig(), bitsightValidSecrets(),
		access.AccessGrant{UserExternalID: "alice@example.com", ResourceExternalID: "analyst"})
