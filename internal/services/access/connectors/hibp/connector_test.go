...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"api_key": "hibpAAAA1234bbbbCCCC"}
}
...
// TestSync_PaginatesUsers verifies HIBP's audit-only contract: SyncIdentities
// invokes the handler exactly once with an empty batch and empty checkpoint
// without touching the network. HIBP has no identity directory to paginate.
// The batch slice must be non-nil per the SyncIdentities empty-batch
// contract documented in access/types.go so downstream consumers that
// JSON-marshal the batch see `[]` rather than `null`.
...
func TestCountIdentities_ValidatesCredentials(t *testing.T) {
	...
	if _, err := c.CountIdentities(context.Background(), validConfig(), map[string]interface{}{}); err == nil {
		t.Error("CountIdentities with missing api_key should error")
	}
