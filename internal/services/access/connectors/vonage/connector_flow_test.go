...
func vonageValidConfig() map[string]interface{} { return map[string]interface{}{} }
func vonageValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "vonage_demo_token"}
}

func TestVonageConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == usersPath:
			...
			_, _ = w.Write([]byte(`{"user":{"id":"` + userID + `","name":"` + email + `","role":"` + role + `"}}`))
		...
		case r.Method == http.MethodGet && (r.URL.Path == userPath || r.URL.Path == emailPath):
			...
			_, _ = w.Write([]byte(`{"user":{"id":"` + userID + `","name":"` + email + `","role":"` + state + `"}}`))
		default:
	...
	cfg := vonageValidConfig()
	secrets := vonageValidSecrets()
...
func TestVonageConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		vonageValidConfig(), vonageValidSecrets(),
		access.AccessGrant{UserExternalID: "alice@example.com", ResourceExternalID: "supervisor"})
