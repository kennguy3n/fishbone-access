...
func TestWazuhGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestWazuhGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://wazuh.example.com/saml/acme/metadata",
		"sso_entity_id":    "https://wazuh.example.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
