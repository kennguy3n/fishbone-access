...
func TestPandaDocCLMGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestPandaDocCLMGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://api.pandadoc.com/saml/acme/metadata",
		"sso_entity_id":    "https://api.pandadoc.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
