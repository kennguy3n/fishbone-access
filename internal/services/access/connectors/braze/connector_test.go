...
func validConfig() map[string]interface{} { return map[string]interface{}{"cluster": "iad-01"} }
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "br_AAAA1234bbbbCCCC"}
}
...
func TestValidate_RejectsMissing(t *testing.T) {
	...
	if err := c.Validate(context.Background(), map[string]interface{}{"cluster": "bad"}, validSecrets()); err == nil {
		t.Error("invalid cluster")
	}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		if r.Header.Get("Accept") != "application/scim+json" {
		...
		body := map[string]interface{}{"totalResults": 2*pageSize - (pageSize - 1)}
		if calls == 1 {
			if startIdx != "1" {
				t.Errorf("startIndex = %q", startIdx)
			}
			...
			body["startIndex"] = 1
			body["itemsPerPage"] = pageSize
		} else {
			if startIdx != fmt.Sprintf("%d", pageSize+1) {
				t.Errorf("startIndex = %q", startIdx)
			}
			body["Resources"] = []map[string]interface{}{{
				"id": "ulast", "userName": "last@x.com", "displayName": "Last", "active": false,
