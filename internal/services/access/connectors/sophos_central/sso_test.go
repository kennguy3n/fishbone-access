...
func TestSophosCentralGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestSophosCentralGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://acme.central.sophos.com/saml/metadata",
		"sso_entity_id":    "https://acme.central.sophos.com/saml",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
