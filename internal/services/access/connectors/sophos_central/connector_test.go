...
func validSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "sc_AAAA1234bbbbCCCC"}
}
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/common/v1/admins" {
		...
		if calls == 1 {
			...
			for i := 0; i < pageSize; i++ {
				arr = append(arr, map[string]interface{}{"id": fmt.Sprintf("u%d", i), "email": fmt.Sprintf("u%d@x.com", i), "firstName": fmt.Sprintf("U%d", i), "lastName": fmt.Sprintf("U%d", i), "status": "active"})
			}
		} else {
			if page != "2" {
				t.Errorf("page = %q", page)
			}
			arr = []map[string]interface{}{{"id": "ulast", "email": "last@x.com", "firstName": "Last", "lastName": "Last", "status": "active"}}
		}
		body["items"] = arr
