...
func TestTrelloGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestTrelloGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://id.atlassian.com/saml/acme/metadata",
		"sso_entity_id":    "https://id.atlassian.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
