...
func TestVultrGetSSOMetadata_NilWithoutURL(t *testing.T) {
	c := New()
...
func TestVultrGetSSOMetadata_WithMetadataURL(t *testing.T) {
	c := New()
	got, err := c.GetSSOMetadata(context.Background(), map[string]interface{}{
		"sso_metadata_url": "https://my.vultr.com/saml/acme/metadata",
		"sso_entity_id":    "https://my.vultr.com/saml/acme",
	}, nil)
	if err != nil {
		t.Fatalf("GetSSOMetadata: %v", err)
	}
