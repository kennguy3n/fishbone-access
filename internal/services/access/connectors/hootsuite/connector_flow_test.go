...
func hootsuiteValidConfig() map[string]interface{} {
	return map[string]interface{}{"org_id": "9999"}
}
func hootsuiteValidSecrets() map[string]interface{} {
	return map[string]interface{}{"token": "hootsuite-token-AAAA"}
}

func TestHootsuiteConnectorFlow_FullLifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		members := "/v1/me/organizations/9999/members"
		memberPath := members + "/" + user
		mu.Lock()
		defer mu.Unlock()
		switch {
	...
	cfg := hootsuiteValidConfig()
	secrets := hootsuiteValidSecrets()
...
func TestHootsuiteConnectorFlow_ProvisionForbiddenFailure(t *testing.T) {
	...
	err := c.ProvisionAccess(context.Background(),
		hootsuiteValidConfig(), hootsuiteValidSecrets(),
		access.AccessGrant{UserExternalID: "alice@example.com", ResourceExternalID: "org_admin"})
