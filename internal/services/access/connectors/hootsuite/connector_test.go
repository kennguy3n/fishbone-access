...
func validConfig() map[string]interface{} { return map[string]interface{}{"org_id": "ORG/123"} }
...
func TestSync_PaginatesUsers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Path-escaped org id should appear in the escaped path verbatim.
		if !strings.Contains(r.URL.EscapedPath(), "ORG%2F123") {
			t.Errorf("expected escaped org id in path; got %q", r.URL.EscapedPath())
		}
		...
		if calls == 1 {
			...
			arr = []map[string]interface{}{{"id": 9999, "email": "last@x.com", "fullName": "Last"}}
		}
		body := map[string]interface{}{"data": arr}
		if next != "" {
