# ShieldNet Access — Design System (WS0 foundation)
...
| `--ok` / `--warn` / `--danger` (+ `-soft`, `-strong`) | Semantic status. |
...
`Toast`, `HelpTooltip`, `Icon`, `AppLayout`. Wrap every async screen in
`AsyncBoundary` so the loading / error / empty lifecycle is consistent.
...
- **WS0 owns** `src/styles.css` tokens and the shared primitives in
  `src/components/`. These are **frozen** after WS0 merges.
