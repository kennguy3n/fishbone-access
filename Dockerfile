...
# all three binaries (ztna-api, access-connector-worker, pam-gateway); each
# compose/Helm service selects one via its command. Alpine (not distroless) is
# used for the final stage so a shell + wget are available for compose
# healthchecks on the single-server tier.
