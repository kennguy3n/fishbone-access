...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| liveness | `GET /health` | 0.31 | 1.44 | 2.39 | 0.51 | 29,157 |
| catalogue | `GET /connectors/providers` (200+) | 0.50 | 1.44 | 2.35 | 0.66 | 23,686 |
| catalogue | `GET /connectors/catalogue/facets` | 2.20 | 2.89 | 4.11 | 2.27 | 6,911 |
| govern | `GET /packs?region=sg` | 2.15 | 2.70 | 3.54 | 2.21 | 7,115 |
| govern | `GET /policies` | 5.31 | 20.72 | 40.03 | 8.44 | 1,818 |
| lifecycle | `GET /access-requests` | 6.44 | 24.96 | 39.65 | 9.29 | 1,676 |
| lifecycle | `GET /connectors` | 10.79 | 24.37 | 42.92 | 12.95 | 1,204 |
| pam | `GET /pam/targets` | 5.86 | 20.46 | 38.21 | 8.26 | 1,854 |
| pam | `GET /pam/leases` | 5.40 | 19.32 | 37.23 | 7.95 | 1,905 |
| pam | `GET /agents` (connector agents) | 6.05 | 22.91 | 33.26 | 8.61 | 1,801 |
| pam | `GET /discovery/summary` (asset inventory) | 11.05 | 29.29 | 42.13 | 14.19 | 1,089 |
| pam | `GET /pam/rotation/policies` | 6.34 | 22.64 | 31.52 | 8.83 | 1,783 |
| pam | `GET /pam/recordings` (searchable index) | 7.19 | 26.50 | 42.73 | 10.52 | 1,456 |
| compliance | `GET /compliance/coverage?framework=SOC 2` | 12.60 | 34.08 | 47.27 | 16.60 | 939 |
| compliance | `GET /compliance/chain/verify` (full, O(n)) | 13.20 | 35.20 | 48.54 | 16.69 | 921 |
| compliance | `GET /compliance/chain/verify?from_seq=…` (incremental, O(Δ)) | 7.93 | 28.53 | 38.49 | 11.36 | 1,360 |
| compliance | `GET /compliance/evidence` | 14.43 | 36.80 | 49.33 | 18.07 | 856 |
| engine | `POST /policies/simulate-definition` | 12.06 | 32.34 | 46.40 | 15.63 | 1,005 |
...
- **Static / in-memory reads are effectively free.** `/health` and the 200+
  provider catalogue answer in **sub-millisecond p50** and push **~24–29k
...
- **The deep privileged-access reads sit in the same band.** The connector-agent
  ...
  one. The asset-discovery summary (`/discovery/summary`, ~11 ms p50) is the
...
- **Compliance is the heaviest read, *by design*.** A full `chain-verify`
  ...
  series sells. Even so, the box serves **~850–940 reads/second** on them.
- **The incremental verify is the scale answer, and it shows up here.** The same
  ...
  At the head — zero new rows — it clocks **7.9 ms p50 / 1,360 req/s**, ~1.5×
...
- **The policy/SoD engine is cheap enough to run inline.** A full dry-run
  ...
  **12.1 ms p50 / 15.6 ms mean**. That matters: it means the `catastrophic`
...
- **Tamper-evidence has a real but bounded cost — with an O(Δ) fast path.**
  ...
  exactly how slow (~13 ms p50, ~920 req/s here). The incremental verify keeps
  the interactive cost flat as the chain grows (7.9 ms p50 / 1,360 req/s),
...
- **The dangerous-grant guardrail is free enough to always be on.** A ~12-ms
